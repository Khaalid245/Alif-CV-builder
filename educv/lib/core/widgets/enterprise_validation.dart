...
class EnterpriseValidators {
  ...
  static EnterpriseValidationResult email(String? value) {
    ...
    if (!emailRegex.hasMatch(value)) {
      return EnterpriseValidationResult.invalid('Please enter a valid email address');
    }
  ...
  static EnterpriseValidationResult password(String? value) {
    ...
    if (value.length < 8) {
      return EnterpriseValidationResult.invalid('Password must be at least 8 characters');
    }
    ...
    if (!hasLower) {
      return EnterpriseValidationResult.invalid('Password needs a lowercase letter');
    }
    if (!hasUpper) {
      return EnterpriseValidationResult.invalid('Password needs an uppercase letter');
    }
    if (!hasDigit) {
  ...
  static EnterpriseValidationResult name(String? value) {
    ...
    if (trimmed.length < 2) {
      return EnterpriseValidationResult.invalid('Name must be at least 2 characters');
    }

    if (trimmed.length > 100) {
      return EnterpriseValidationResult.invalid('Name must not exceed 100 characters');
    }

    // Check for valid characters (letters, spaces, hyphens, apostrophes)
    final nameRegex = RegExp(r"^[a-zA-Z\s\-']+$");
    if (!nameRegex.hasMatch(trimmed)) {
      return EnterpriseValidationResult.invalid('Name contains invalid characters');
    }
  ...
  static EnterpriseValidationResult required(String? value, [String? fieldName]) {
    if (value == null || value.trim().isEmpty) {
      return EnterpriseValidationResult.invalid('${fieldName ?? 'This field'} is required');
    }
    return EnterpriseValidationResult.valid();
  }

  // URL validation
  static EnterpriseValidationResult url(String? value, {bool required = false}) {
    if (value == null || value.isEmpty) {
      if (required) {
  ...
  static EnterpriseValidationResult date(String? value) {
    ...
    try {
      ...
      if (date.isAfter(now)) {
        return EnterpriseValidationResult.invalid('Date cannot be in the future');
      }
  ...
  static EnterpriseValidationResult confirmPassword(String? value, String? originalPassword) {
    if (value == null || value.isEmpty) {
  ...
  // Length validation
  static EnterpriseValidationResult maxLength(String? value, int maxLength, [String? fieldName]) {
  ...
  static EnterpriseValidationResult minLength(String? value, int minLength, [String? fieldName]) {
  ...
  static EnterpriseValidationResult combine(List<EnterpriseValidationResult> results) {
    ...
    return results.isNotEmpty ? results.last : EnterpriseValidationResult.valid();
  }
}
...
class EnterpriseValidatedInput extends StatefulWidget {
  ...
  @override
  State<EnterpriseValidatedInput> createState() => _EnterpriseValidatedInputState();
}

class _EnterpriseValidatedInputState extends State<EnterpriseValidatedInput> {
  EnterpriseValidationResult _validationResult = EnterpriseValidationResult.initial();
  ...
  Widget build(BuildContext context) {
    return AccessibleInput(
      ...
      validator: _hasBeenTouched ? (value) => _validationResult.errorMessage : null,
  ...
  String _buildSemanticLabel() {
    ...
    if (_validationResult.state == ValidationState.invalid) parts.add('invalid');
